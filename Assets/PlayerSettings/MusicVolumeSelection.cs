...
namespace PlayerSettings {
    public class MusicVolumeSelection : VolumeSelectionHandler {
        ...
        public void OnVolumeChanged() {
            ...
            PlayerDataManager.Instance.musicMixer.audioMixer.SetFloat("MusicVolume", SelectedVolume);
        }
        
        public void OnExitSettings() {
            OriginalVolume = PlayerDataManager.Instance.auditory.musicVolume;
        ...
        private void AddToLoadEvent() {
            if (!Mathf.Approximately(SelectedVolume, OriginalVolume)) {
                EventBus<AddChangeEvent>.Publish(new AddChangeEvent {
                    ChangeLog = $"Music Volume: {OriginalVolume} -> {SelectedVolume}\n"
                });
        ...
        private void LoadAuditorySetting() {
            PlayerDataManager.Instance.auditory.musicVolume = SelectedVolume;
            OnExitSettings();
        }
    }
}
