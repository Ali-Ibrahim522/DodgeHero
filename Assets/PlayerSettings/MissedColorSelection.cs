...
namespace PlayerSettings {
    public class MissedColorSelection : ColorSelectionHandler {
        ...
        public void OnExitSettings() {
            OriginalColor = PlayerDataManager.Instance.visuals.missed;
        ...
        private void AddToLoadEvent() {
            if (SelectedColor != OriginalColor) {
                EventBus<AddChangeEvent>.Publish(new AddChangeEvent {
                    ChangeLog = $"Missed Color: #{ColorUtility.ToHtmlStringRGBA(OriginalColor)} -> #{ColorUtility.ToHtmlStringRGBA(SelectedColor)}\n"
                });
        ...
        private void LoadVisualSetting() {
            PlayerDataManager.Instance.visuals.missed = SelectedColor;
            OnExitSettings();
        }
    }
}
