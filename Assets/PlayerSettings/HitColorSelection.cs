...
namespace PlayerSettings {
    public class HitColorSelection : ColorSelectionHandler {
        ...
        public void OnExitSettings() {
            OriginalColor = PlayerDataManager.Instance.visuals.hit;
        ...
        private void AddToLoadEvent() {
            if (SelectedColor != OriginalColor) {
                EventBus<AddChangeEvent>.Publish(new AddChangeEvent {
                    ChangeLog = $"Hit Color: #{ColorUtility.ToHtmlStringRGBA(OriginalColor)} -> #{ColorUtility.ToHtmlStringRGBA(SelectedColor)}\n"
                });
        ...
        private void LoadVisualSetting() {
            PlayerDataManager.Instance.visuals.hit = SelectedColor;
            OnExitSettings();
        }
    }
}
